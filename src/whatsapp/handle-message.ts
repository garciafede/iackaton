import {compareCart,formatCart,MAX_CART_ITEMS} from '../ai/cart.js';
...
async function processConversation(message:WhatsAppMessage,d:WhatsAppWebhookDependencies):Promise<void>{
  ...
  const log=async(direction:'IN'|'OUT',text?:string,error?:string)=>{
    ...
    catch{console.warn(JSON.stringify({event:'conversation.log_unavailable',sessionId:state.sessionId}));}
  };
  const send=async(body:string)=>{await d.sendText(from,body);await log('OUT',body);};
  ...
  async function searchProduct(query:string,followup=false){
    state.activeSubject='product';
    if(!followup)state.lastProduct={query,sort:state.sortCriterion,radiusKm:resolveMessageRadius(query)};
    save();
    if(!state.location){state.pendingAction={...(state.pendingAction?.type==='LOCATION'?state.pendingAction:{}),type:'LOCATION',resume:'product'};save();await send((await d.agent({message:query})).message);return;}
    if(!state.lastProduct){await send('Decime qué producto querés comparar.');return;}
    delete state.lastProductResults;
    const input=followup?(state.sortCriterion==='price'?'más barata':state.sortCriterion==='distance'?'más cercana':query):query;
  ...
  async function searchCart(text:string){
    ...
    const result=wantsDetails(text)&&state.cartResults?state.cartResults:await(d.cartSearch??compareCart)(state.currentCart,state.location.latitude,state.location.longitude,state.sortCriterion,resolveMessageRadius(text));
  ...
  try{
    ...
    switch(intent.name){
      ...
      case 'CART_FOLLOWUP':await searchCart(text);return;
