...
export function formatCompactOffers(data:{product:{name:string;size:string|null};results:SearchResult[]},sort:SearchSort):string {
  ...
  const title=[clean(data.product.name), clean(data.product.size ?? "")].filter(Boolean).join(" ");
  ...
  return [`*${title}*`,`${heading}:`,...lines,online?"*Precio online; precio y disponibilidad local no confirmados.":pickup?"*Compra online con retiro; disponibilidad en góndola no confirmada.":"*Precio relevado; disponibilidad no confirmada."].join("\n");
}
