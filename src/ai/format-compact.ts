...
export function formatCompactOffers(data:{product:{name:string;size:string|null};results:SearchResult[]},sort:SearchSort):string {
  ...
  const multiple=new Set(rows.map(x=>x.product?.ean??`${x.product?.name}:${x.product?.variant}:${x.product?.size}`)).size>1;
  const lines=rows.map((r,i)=>`${i+1}. ${clean(r.store.name)} — $${money(r.price)} · ${r.distanceKm.toLocaleString("es-AR",{maximumFractionDigits:2})} km${r.live?.priceScope==="BRANCH_CONFIRMED"?" · retiro disponible":""}${r.source==="DEMO"?" · DEMO":""}${multiple?`\n${clean(r.product?.name??data.product.name)} ${clean(r.product?.size??"")}`:""}`);
